...
class Encoder(nn.Module):
    def __init__(self, in_nc, mid_nc, out_nc, prior_nc, sft_ks):
        ...
        self.g_a6 = conv3x3(mid_nc[0], out_nc)
...
class LFGCM(CompressionModel):
    ...
    def forward(self, x, ref):
        ...
        # Hyper-parameters
        hyper_params = self.hyper_dec(z_hat) # fai
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                # Anchor
                params_anchor = self.entropy_parameters_anchor[idx](hyper_params) # EntropyParametersEX
                ...
                _, y_slice_likelihoods = self.gaussian_conditional(y_slice, scales_slice, means_slice) # GaussianConditional
        ...
        #ref = self.f_c(z_hat)
        output = self.decoder(y_hat, None)
